...
export default function ArchetypePageView({ players, archetype }) {
  const { lang, t } = useLang();
  const { league } = useLeague();
  const filtered = players.filter((p) => p.lig === league);
  ...
  return (
    <div className="wrap ow">
      ...
      <h1 className="ow__title">{archetype}</h1>
      <p className="ow__intro">{t.archetypePageIntro(archetype)}</p>
      <div className="ow__cards" style={{ marginTop: 28 }}>
        {filtered.map((p) => (
          <Link key={p.slug} href={playerHref(p.slug, lang)} className="ow__card" style={{ '--ring': p.takimRenk }}>
            ...
            <span className="ow__card-badge">{archetype}</span>
